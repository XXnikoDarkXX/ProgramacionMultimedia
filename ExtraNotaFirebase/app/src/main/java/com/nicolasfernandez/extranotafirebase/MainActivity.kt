...
class MainActivity : AppCompatActivity() {
    ...
    fun clickBotonBorrar(view: View) {
        val selfRef=this;
        firebaseDB.collection("prueba").document("a").delete().addOnCompleteListener {
            task->
            if (task.isSuccessful){
                Toast.makeText(selfRef,"Se ha borrado el documento a correctamente",Toast.LENGTH_LONG).show()
            }else{
